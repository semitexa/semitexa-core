...
- **GitHub Security Advisories** — [open a private report](https://github.com/semitexa/semitexa-core/security/advisories/new).
  This keeps the discussion, the fix and the eventual advisory in one place, and
  nothing is visible until we publish it.
- **Email** — `support@semitexa.com`, the address this package already declares
  in its `composer.json`. Put "security" in the subject.
...
In scope: anything in this package that lets a request do something the
application did not authorise — the DI container and its discovery, routing and
payload hydration, the console entry points, the Composer plugin, and the
framework's own HTTP handling.
